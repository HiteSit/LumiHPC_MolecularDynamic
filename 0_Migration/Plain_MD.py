...
class Molecular_Dynamics:
    ...
    @staticmethod
    def combine_topology(protein_fixer, lig_top, system_generator):
    ...
    def _ligand_preparation(self):
        ligand_mol = Molecule.from_file(self.sdf_path)
    ...
    def system_generator(self, init_temp, delta_pico):
        ...
        system_generator = SystemGenerator(forcefields=['amber/ff14SB.xml', 'amber/tip3p_standard.xml'],
                                           small_molecule_forcefield='gaff-2.11', molecules=[ligand_mol],
                                           forcefield_kwargs=forcefield_kwargs)

        # Grab the general topology of the complex
        modeller = self.combine_topology(protein_fixer, ligand_top, system_generator)

        self.system = system_generator.create_system(modeller.topology, molecules=ligand_mol)
